...
export function WalletDropdown(): JSX.Element {
    ...
    async (address: string, forceRefresh = false) => {
      ...
      try {
        ...
        try {
          ...
          txHistory = [
            {
              signature: "5vjTDurRrxv3WkFXbCxdYu6aEPj9vV",
              type: "Received",
              amount: "0.05 SOL",
              date: new Date(Date.now() - 86400000).toLocaleString(),
              counterparty: "8Kv4C...j9vV",
              tokenType: "SOL",
            },
            {
              signature: "4wxs15LhEqPZfNbsBvRsJsLcLNhJzxD",
              type: "Sent",
              amount: "10.50 USDC",
              date: new Date(Date.now() - 172800000).toLocaleString(),
              counterparty: "DGHBr...dPEZ",
              tokenType: "USDC",
            },
          ];
        }
  ...
  useEffect(() => {
    const initWallet = async (): Promise<void> => {
      if (authenticated && ready && isOpen) {
        try {
          ...
          const wallet = getSolanaEmbeddedWallet(wallets);
    ...
    initWallet();
  }, [authenticated, ready, wallets, isOpen, fetchWalletData, isCacheValid]);
                    ...
                    <ScrollArea className="h-[300px]">
                      {walletState.transactions.length > 0 ? (
                        <div className="space-y-3">
                          {walletState.transactions.map((tx, index) => (
                                ...
                                <div className="flex justify-between items-start">
                                  ...
                                  <div className="text-right">
                                    ...
                                    <a
                                      ...
                                      className="text-xs flex items-center justify-end mt-1 text-blue-600 hover:underline"
                                    >
                                      View{" "}
